...
// 多列表
class STypeSizeRow : public SMultiColumnTableRow<FListViewItemPtr>
{
	SLATE_BEGIN_ARGS(STypeSizeRow) :
		_ItemShow()
	{}
	SLATE_ARGUMENT(FListViewItemPtr, ItemShow)
		SLATE_END_ARGS()
public:
...
TSharedPtr<SWidget> FCommonWidget::MakeWidget()
{
	...
	+ SVerticalBox::Slot()
		.AutoHeight()
		[
			SAssignNew(ThisListView, SListView<FListViewItemPtr>)
			.ItemHeight(50)//Item高度
			.ListItemsSource(&this->ItemData)//要显示的数据
			.OnGenerateRow_Lambda([](FListViewItemPtr InItem, const TSharedRef<STableViewBase>& OwnerTable)->TSharedRef<ITableRow> {
		...
		.OnSelectionChanged_Lambda([this](FListViewItemPtr inSelectItem, ESelectInfo::Type SelectType)
		{
			...
			{
				TArray<FListViewItemPtr> selectItems = ThisListView->GetSelectedItems();
